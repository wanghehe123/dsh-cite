...
- 引用前文：在会话正文中选中文本会出现「添加可选评论…」输入框 + 圆形「引用 N」确认按钮；评论可留空，引用以输入框 chip + 上方引用条暂存，支持展开/收起与逐条移除，发送时自动序列化为 `> ` 引用块 + 可选评论（详见[引用前文](#引用前文codex-式选区引用)）。
...
1. 在聊天正文中选中一段文本（代码、自然语言均可）。选区上方会出现浮动卡片：左侧是「添加可选评论…」输入框，右侧是蓝色圆形按钮，显示将要成为的「引用 N」序号。
2. 可选的评论先 `trim`，超过 4,000 Unicode 码点时截断并追加截断标记；然后按 Enter 或点圆形按钮：
   - 引用文本先 `trim`，超过 16,000 Unicode 码点时截断并追加本地化截断标记；
   - 评论为空时行为与无评论引用完全一致；
   - 在草稿末尾插入一个 `dsh-sessions-quote` chip，输入框内显示「引用 N」小标签，评论存在 chip 的 ref 里。
3. 输入框上方出现引用条：
   - 列出当前草稿中所有引用 chip；
   - 每条显示「引用 N」、首行预览；
   - 可展开/收起查看全文，有评论的引用展开后会追加「评论」小节；
   - 可逐条移除（等价于在输入框里删除对应 chip），评论随引用一并移除。
4. 继续输入正文或直接发送。发送时每个 chip 被序列化为逐行 `> ` 前缀的 Markdown 引用块；有评论时在引用块后空一行追加评论，模型收到的就是引用内容加你的说明/追问。
5. 发送成功后草稿清空，引用条消失；如果引用被移除或手动删除 chip，引用条也会同步消失。

#### 交互与实现细节

- **触发条件**：仅当选区位于会话正文（`[data-conversation-scroll]` 内）且不在输入区/引用条（`[data-composer-seat]` 内）时显示浮动卡片；空选区、输入阶段非 `plain` 时不显示。
- **评论输入不丢选区**：卡片 `mousedown` 阻止默认行为并手动聚焦输入框，输入期间暂停 `selectionchange` 重定位；Enter 在 IME 组词中只提交组词、不触发添加，Esc 或点击外部关闭并丢弃未确认评论；插入失败会保留已输入评论，1.4 秒后可直接重试。
